...
describe("pow2.editor.TilePaintAction",()=>{
   ...
   describe('constructor',()=>{
      it('should throw an error if invalid arguments are provided',()=>{
         expect(() => {
            new pow2.editor.TilePaintAction(null,null,null);
         }).toThrow(new Error(pow2.errors.INVALID_ARGUMENTS));
         expect(() => {
            new pow2.editor.TilePaintAction(null,1337,1);
         }).toThrow(new Error(pow2.errors.INVALID_ARGUMENTS));
         expect(() => {
            new pow2.editor.TilePaintAction(fix.editor.tileMap.layers[0],1337,1);
   ...
   describe('execute',()=>{
      it('should set the texture gid for the tile',()=>{
         ...
         var act = new pow2.editor.TilePaintAction(fix.editor.tileMap.layers[0],0,0);
         act.execute();
         expect(layer.tiles[0]).toBe(0);
      });
      it('should fail if parent does',()=>{
         var act = new pow2.editor.TilePaintAction(fix.editor.tileMap.layers[0],0,1);
         act.execute();
